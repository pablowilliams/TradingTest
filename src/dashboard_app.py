...
SECRET = os.getenv("SECRET_KEY", "change-me-to-a-random-string")
ADMIN_USER = os.getenv("DASHBOARD_USER", "admin")
ADMIN_PASS_HASH = hashlib.sha256(os.getenv("DASHBOARD_PASSWORD", "tradingtest").encode()).hexdigest()


def _check_auth(request: Request) -> bool:
    token = request.cookies.get("auth_token", "")
    expected = hashlib.sha256(f"{SECRET}:{ADMIN_PASS_HASH}".encode()).hexdigest()
    return token == expected


def _auth_token() -> str:
    return hashlib.sha256(f"{SECRET}:{ADMIN_PASS_HASH}".encode()).hexdigest()
...
<body><div class="login-box">
    ...
    async def do_login(username: str = Form(...), password: str = Form(...)):
        ...
        if username == ADMIN_USER and pw_hash == ADMIN_PASS_HASH:
            ...
            resp.set_cookie("auth_token", _auth_token(), httponly=True, max_age=86400)
            return resp
...
<h2 style="color:#f44336">Invalid credentials</h2><a href="/login">Try again</a>
    ...
    async def dashboard(request: Request):
        ...
        for s in stats:
            ...
            bot_rows += f"""<tr>
                <td>{s['bot_id']}</td><td>{s['total_trades']}</td><td>{s['wins']}</td>
        ...
        for t in recent:
            ...
            trade_rows += f"""<tr>
                <td>{t['bot_id']}</td><td>{t.get('market_type','?')}</td><td>{t['outcome']}</td>
                <td>${t['amount']:.2f}</td><td>{t['confidence']:.1%}</td>
                <td style="color:{pc}">${t.get('pnl',0):+.2f}</td><td>{t['status']}</td></tr>"""
